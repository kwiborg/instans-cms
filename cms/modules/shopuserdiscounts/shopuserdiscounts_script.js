function select_productgroup(pgroup_id){
...
function verifyuserprices(obj){
	...
	for(i=0; i<allfields.length; i++){
		temp = allfields[i].name.split("_");
		if (temp[0] == "userprice"){
			$(allfields[i].id).style.border = "1px solid #060";
			if (isNaN(allfields[i].value) && allfields[i].value != ""){
				alert("Fejl i værdi: '" + allfields[i].value + "' er ikke en gyldig numerisk pris. Tryk OK og se efter den røde boks.");
				errors += 1;
				$(allfields[i].id).style.border = "2px solid #f00";
				document.location = "#scrollto_" + allfields[i].id;
				return;
			}
		}
	}
	document.forms[0].dothis.value = "saveuserprices";
	document.forms[0].submit();
}

function userprice_format(obj){
	obj.value = obj.value.replace(/,/, ".");
	verifyuserprices(obj);
}

function usersearchtoggle(obj){
	if (trim(obj.value) != ""){
		$("usergroupselector").disabled = true;
		$("usergroupselector").value = 0;
	} else {
		$("usergroupselector").disabled = false;
	}
}
...
function ResizeCells(table, cornerDiv, headerRowDiv, headerColumnDiv, bodyDiv)
{
	...
	//alert(headerRowColumns[0].childNodes);
	...
	for (i = 0; i < headerRowColumns.length; i++){
		//alert(i + " " + headerRowColumns[i].id);
	}

	for (i = 0; i < headerRowColumns.length; i++)
	{
		//alert(headerRowColumns.length);
		//alert(i + " " + headerRowColumns[i].id);
