...
$font_file  = 'fonts/headingfont.ttf' ;
$font_size  = 22 ;
$lineheight = 36; // Set to 0 for auto-lineheight. Else set to line height which must be > font_size
$font_color = '#000000' ;
$background_color = '#ffffff' ;
$transparent_background  = true ;
$cache_images = false ;
$cache_folder = 'images/gfxheadings_cache' ;
...
// Make sure image is always exactly same height as line if lineheight > 0 ;
if ($lineheight > 0) {
	$image = @ImageCreate(abs($box[2]-$box[0]),$lineheight) ;
} else {
...
// Adjust position of text in image if lineheight is not auto (> 0);
$calculated_pos_autolineheight = abs($box[5]-$box[3])-$box[1];
$calculated_imageheight_autolineheight = abs($box[5]-$dip);
$actual_imageheight = imagesy($image);
$offset = $actual_imageheight-$calculated_imageheight_autolineheight;
$new_pos = $calculated_pos_autolineheight+$offset;
if ($lineheight > 0) {
	ImageTTFText($image,$font_size,0,-$box[0],$new_pos,
    	$font_color,$font_file,$text) ;
} else {
	ImageTTFText($image,$font_size,0,-$box[0],$calculated_pos_autolineheight,
    	$font_color,$font_file,$text) ;
}
